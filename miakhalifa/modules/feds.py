...
from miakhalifa import dispatcher, OWNER_ID, SUDO_USERS, DEV_USERS, WHITELIST_USERS, LOGGER, MESSAGE_DUMP
from miakhalifa.modules.helper_funcs.handlers import CMD_STARTERS
from miakhalifa.modules.helper_funcs.misc import is_module_loaded, send_to_list
from miakhalifa.modules.helper_funcs.chat_status import is_user_admin
from miakhalifa.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from miakhalifa.modules.helper_funcs.string_handling import markdown_parser
from miakhalifa.modules.disable import DisableAbleCommandHandler

import miakhalifa.modules.sql.feds_sql as sql
...
#Further Edited by @TheUnusualPsychopath
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "miakhalifa_fbanned_users.json"
                update.effective_message.reply_document(document=output, filename="miakhalifa_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "miakhalifa_fbanned_users.csv"
                update.effective_message.reply_document(document=output, filename="miakhalifa_fbanned_users.csv",
